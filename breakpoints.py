...
def main():
    ...
    for name in autosets:
        ...
        result[name]['F-score'] = 2 * (result[name]['Precision'] * result[name]['Recall']
                                       / (result[name]['Precision'] + result[name]['Recall']))

    df = pd.DataFrame.from_dict(result, orient='index')
    df.to_excel('/home/al/PythonFiles/files/disser/breakpoints_Series3.xlsx')
